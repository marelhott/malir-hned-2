...
const EMAIL_TIMEOUT_MS = Math.max(1000, Number(process.env.EMAIL_TIMEOUT_MS || '4000') || 4000)
...
<div class="wrap">
  ...
  <div class="foot">Malíř Hned · Praha a Středočeský kraj · <a href="https://malirhned.cz" style="color:#1a6a3a">malirhned.cz</a></div>
</div>
</body></html>`
}
...
/**
 * Malíři — zakázka s jejich nabídkou/přiřazením byla zrušena
 */
export async function sendPainterJobCancelled({ painter, job, portalUrl }) {
  const subject = `❌ Zakázka ${job.reference} byla zrušena`
  const html = wrap('Zakázka zrušena', `
    <h2>Zakázka byla zrušena, ${painter.name}</h2>
    <p>Zakázka, ke které jste se vztahuje vaše nabídka, byla zrušena zákazníkem nebo dispečinkem.</p>
    <div class="box">
      <strong>Číslo zakázky</strong>${job.reference}
      <strong style="margin-top:10px">Typ práce</strong>${job.work_type || '—'}
      <strong style="margin-top:10px">Termín</strong>${job.preferred_date_label || job.preferred_date || '—'}
      <strong style="margin-top:10px">Oblast</strong>${job.locality || '—'}
    </div>
    <p>Tento termín je nyní opět ve vašem kalendáři volný. Nové nabídky vám pošleme co nejdříve.</p>
