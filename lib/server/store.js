...
class SupabaseStorageStore {
  ...
  async writeState(state) {
    ...
    const { error } = await this.client.storage
      .from(this.bucket)
      .upload(this.objectPath, new Blob([payload], { type: 'application/json' }), {
        upsert: true,
  ...
  async listJobs() {
    const state = await this.readState()
    return state.jobs.map((job) => buildJobSummary(job, state.job_offers, state.job_events))
  }
  ...
  async getJob(jobId) {
    ...
    return {
      job,
      offers: state.job_offers.filter((offer) => offer.job_id === jobId),
      ...
      notifications: state.notifications.filter((item) => item.job_id === jobId),
    }
  }

  async getPainters() {
    const state = await this.readState()
    return state.painters.filter((painter) => painter.is_active)
  }
  ...
  async getOfferByToken(token) {
    ...
    return {
      offer,
      job: state.jobs.find((item) => item.id === offer.job_id),
      painter: state.painters.find((item) => item.id === offer.painter_id),
    }
  }
  ...
  async respondToOffer(token, decision) {
    ...
    if (new Date(offer.expires_at).getTime() < Date.now()) {
      offer.status = OFFER_STATUSES.EXPIRED
      offer.updated_at = nowIso()
      state.job_events.push(createEvent({ jobId: job.id, offerId: offer.id, eventType: EVENT_TYPES.OFFER_EXPIRED, actorType: 'system', actorLabel: 'System' }))
      await this.writeState(state)
      throw new Error('Tato nabídka už není dostupná.')
    }
    ...
    if (decision === 'accept') {
      ...
      job.selected_painter_name = offer.painter_name
      job.updated_at = nowIso()
      state.job_events.push(createEvent({ jobId: job.id, offerId: offer.id, eventType: EVENT_TYPES.OFFER_ACCEPTED, actorType: 'painter', actorLabel: offer.painter_name }))
    } else {
      offer.status = OFFER_STATUSES.DECLINED
      offer.responded_at = nowIso()
      offer.updated_at = nowIso()
      job.status = JOB_STATUSES.READY_TO_OFFER
      job.updated_at = nowIso()
      state.job_events.push(createEvent({ jobId: job.id, offerId: offer.id, eventType: EVENT_TYPES.OFFER_DECLINED, actorType: 'painter', actorLabel: offer.painter_name }))
    }
  ...
  async confirmAssignment(jobId, adminEmail) {
    ...
    for (const offer of state.job_offers.filter((item) => item.job_id === jobId && item.id !== winningOffer.id && item.status === OFFER_STATUSES.PENDING)) {
      offer.status = OFFER_STATUSES.WITHDRAWN
      offer.updated_at = nowIso()
      state.job_events.push(createEvent({ jobId, offerId: offer.id, eventType: EVENT_TYPES.OFFER_WITHDRAWN, actorType: 'system', actorLabel: 'System' }))
    }
    job.status = JOB_STATUSES.CLIENT_CONFIRMED
    job.assigned_at = nowIso()
    job.updated_at = nowIso()
    state.job_events.push(
      createEvent({ jobId, offerId: winningOffer.id, eventType: EVENT_TYPES.JOB_ASSIGNED, actorType: 'admin', actorLabel: adminEmail, payload: { painterName: winningOffer.painter_name } }),
      createEvent({ jobId, offerId: winningOffer.id, eventType: EVENT_TYPES.CLIENT_ASSIGNED_NOTIFICATION_QUEUED, actorType: 'system', actorLabel: 'System' }),
      createEvent({ jobId, offerId: winningOffer.id, eventType: EVENT_TYPES.PAINTER_CONTACT_UNLOCKED, actorType: 'system', actorLabel: 'System' }),
    )
    await this.writeState(state)
    return job
  }
  ...
  async returnToDispatch(jobId, adminEmail) {
    ...
    job.selected_painter_name = null
    job.updated_at = nowIso()
    state.job_events.push(createEvent({ jobId, eventType: EVENT_TYPES.JOB_RETURNED_TO_DISPATCH, actorType: 'admin', actorLabel: adminEmail }))
    await this.writeState(state)
    return job
  }
  ...
  async markJobState(jobId, adminEmail, nextStatus) {
    ...
    const eventType =
      ...
      nextStatus === JOB_STATUSES.DONE ? EVENT_TYPES.JOB_COMPLETED :
      EVENT_TYPES.JOB_REVIEWED
    state.job_events.push(createEvent({ jobId, eventType, actorType: 'admin', actorLabel: adminEmail }))
    await this.writeState(state)
    return job
  }
  ...
  async cancelAdminJob(jobId, adminEmail) {
    ...
    job.status = JOB_STATUSES.CANCELED
    job.updated_at = nowIso()
    for (const offer of state.job_offers.filter((item) => item.job_id === job.id && item.status === OFFER_STATUSES.PENDING)) {
      offer.status = OFFER_STATUSES.WITHDRAWN
      offer.updated_at = nowIso()
      state.job_events.push(createEvent({ jobId: job.id, offerId: offer.id, eventType: EVENT_TYPES.OFFER_WITHDRAWN, actorType: 'system', actorLabel: 'System' }))
    }
    state.job_events.push(createEvent({ jobId: job.id, eventType: EVENT_TYPES.JOB_CANCELED, actorType: 'admin', actorLabel: adminEmail }))
    await this.writeState(state)
    return job
  }
  ...
  async getPublicJob(token) {
    ...
    return {
      job,
      offers: state.job_offers.filter((offer) => offer.job_id === job.id),
      events: state.job_events.filter((event) => event.job_id === job.id),
    }
  }
  ...
  async completePublicJob(token, patch) {
    ...
    job.status = JOB_STATUSES.WAITING_REVIEW
    job.updated_at = nowIso()
    state.job_events.push(createEvent({ jobId: job.id, eventType: EVENT_TYPES.CLIENT_COMPLETED, actorType: 'client', actorLabel: job.client_name || 'Klient' }))
    await this.writeState(state)
    return job
  }
  ...
  async cancelPublicJob(cancelToken) {
    ...
    state.job_events.push(createEvent({ jobId: job.id, eventType: EVENT_TYPES.JOB_CANCELED, actorType: 'client', actorLabel: job.client_name || 'Klient' }))
    await this.writeState(state)
    return job
  }
}
...
export function getStore() {
  ...
  if (hasSupabaseConfig() && getSupabaseStoreMode() === 'storage') {
