...
function normalizeState(state) {
  ...
  state.painters = DEFAULT_PAINTERS.map((defaultPainter) => {
    ...
    const merged = {
      ...
      email: defaultPainter.email || item.email,
      phone: defaultPainter.phone || item.phone,
...
class JsonStateStore {
  ...
  async createJob(payload, req) {
    ...
    // Emaily — fire-and-forget, nesmí blokovat odpověď
    const baseUrl = getAppBaseUrl()
    const adminUrl = `${baseUrl}/admin`
    if (job.client_email) {
      sendJobReceived({ job, publicUrl: links.publicUrl, cancelUrl: links.cancelUrl }).catch(() => {})
    }
    sendAdminNewJob({ job, adminUrl }).catch(() => {})
  ...
  async sendOffer(jobId, painterId, adminEmail, req) {
    ...
    // Email malíři s odkazem na nabídku
    if (painter.email) {
      sendPainterOffer({ painter, job, offerUrl: `${getAppBaseUrl(req)}/nabidka?token=${token}`, expiresAt }).catch(() => {})
    }
  ...
  async confirmAssignment(jobId, adminEmail) {
    ...
    if (job.client_email && winningPainter) {
      ...
      sendJobAssigned({ job, painter: winningPainter, publicUrl }).catch(() => {})
    }
    if (winningPainter?.email) {
      const portalToken = winningPainter.portal_access_token
      const portalUrl = portalToken ? `${baseUrl}/maliri?token=${portalToken}` : `${baseUrl}/maliri`
      sendPainterContactUnlocked({ painter: winningPainter, job, portalUrl }).catch(() => {})
    }
  ...
  async getPainterPortal(token) {
    ...
    const jobs = state.jobs
      .filter((job) => job.assigned_painter_id === painter.id)
      .sort((a, b) => new Date(b.created_at) - new Date(a.created_at))
    return { painter, availability, offers, jobs }
  }
