...
class JsonStateStore {
  ...
  async createJob(payload, req) {
    ...
    // Emaily — await both so Vercel doesn't freeze the process before they complete
  ...
  async sendOffer(jobId, painterId, adminEmail, req) {
    ...
    // Email malíři — await so Vercel doesn't kill process before send
    if (painter.email) {
      await sendPainterOffer({ painter, job, offerUrl: `${getAppBaseUrl(req)}/nabidka?token=${token}`, expiresAt })
    }
...
class SupabaseDbStore extends JsonStateStore {
  ...
  async writeRawState(state) {
    ...
    await Promise.all([
      ...
      this._upsert('job_events', events),
      this._upsert('notifications', notifs),
