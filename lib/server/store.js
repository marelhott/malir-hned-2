...
function normalizeState(state) {
  ...
  const defaultPaintersByIndex = DEFAULT_PAINTERS
  state.jobs = Array.isArray(state.jobs) ? state.jobs : []
  state.job_photos = Array.isArray(state.job_photos) ? state.job_photos : []
  state.painters = (Array.isArray(state.painters) ? state.painters : DEFAULT_PAINTERS).map((item, index) => {
    const defaultPainter = defaultPaintersByName.get(item.name) || defaultPaintersByIndex[index] || {}
    const merged = {
      ...defaultPainter,
      id: item.id || createId(),
      ...item,
