...
const UNKNOWN_AVAILABILITY_STATUS = 'unknown'
...
function hashPainterPin(pin, { salt = crypto.randomBytes(16).toString('hex'), temporary = true } = {}) {
  const digest = crypto.scryptSync(String(pin), salt, 32).toString('hex')
  return `${temporary ? 'pin' : 'pin2'}$${salt}$${digest}`
}

function isPainterPinHash(value) {
  return typeof value === 'string' && /^(pin|pin2)\$[^$]+\$[a-f0-9]+$/i.test(value)
}

function isTemporaryPainterPinHash(value) {
...
function runInBackground(label, task) {
  ...
  try {
    ...
    // Local/dev fallback outside Vercel runtime.
  }
}
...
function normalizePainter(painter) {
  ...
  if (!isPainterPinHash(next.portal_token_hash)) {
    next.portal_token_hash = hashPainterPin(deriveDefaultPainterPin(next), { temporary: true })
  }
...
function buildPainterPortalAvailability(state, painterId, startDate = todayIso(), days = 180) {
  return Array.from({ length: days }, (_, offset) => {
    const date = addDays(startDate, offset)
    const row = getAvailabilityForDate(state, painterId, date) || {
      id: null,
      painter_id: painterId,
      date,
      status: UNKNOWN_AVAILABILITY_STATUS,
      capacity: 0,
...
function buildSuggestedPainter(state, job, painter) {
  ...
  const status = availability?.status || UNKNOWN_AVAILABILITY_STATUS
  let score = 0
  ...
  if (status === UNKNOWN_AVAILABILITY_STATUS) score -= 4
  ...
  return {
    ...
    display_status:
      ...
      status === UNKNOWN_AVAILABILITY_STATUS ? 'Bez zadané dostupnosti' :
      'Nedostupný',
  }
}
...
function buildPublicCalendar(state, options = {}) {
  ...
  for (let offset = 0; offset < days; offset += 1) {
    ...
    const suitablePainters = state.painters.filter((painter) => {
      ...
      const hasConfirmedBlock = getActiveBlocksForDate(state, painter.id, date)
        .some((row) => row.block_type === CAPACITY_BLOCK_TYPES.CONFIRMED_ASSIGNMENT)
      const effectiveStatus = availability?.status || UNKNOWN_AVAILABILITY_STATUS

      if (options.express === 'true' && !(availability?.accepts_express || painter.is_express_enabled)) return false

      // Public web po čistém resetu počítá "unknown" jako výchozí dostupnost.
      // Jinak by první kontrola remainingCapacity celý fallback zabila, protože
      // malíř bez explicitního řádku availability vrací 0.
      if (effectiveStatus === UNKNOWN_AVAILABILITY_STATUS) {
        return !hasConfirmedBlock
      }

      if (effectiveStatus === AVAILABILITY_STATUSES.UNAVAILABLE) return false
      if (hasConfirmedBlock || remainingCapacity <= 0) return false
...
function buildDayPainters(state, date, job = null) {
  return state.painters
    ...
    .map((painter) => {
      ...
      const status = availability?.status || UNKNOWN_AVAILABILITY_STATUS
      const blocked = activeBlocks.length > 0
      const visualStatus = blocked
...
function buildAdminAvailabilityCalendar(state, options = {}) {
  ...
  for (let offset = 0; offset < totalDays; offset += 1) {
    ...
    const availableCount = painters.filter((item) => item.visual_status === AVAILABILITY_STATUSES.AVAILABLE).length
    const limitedCount = painters.filter((item) => item.visual_status === AVAILABILITY_STATUSES.LIMITED).length
    const unavailableCount = painters.filter((item) => item.visual_status === AVAILABILITY_STATUSES.UNAVAILABLE).length
    const unknownCount = painters.filter((item) => item.visual_status === UNKNOWN_AVAILABILITY_STATUS).length
    ...
    let serviceLabel = 'Bez zadání'
    if (unknownCount === painters.length && blockedCount === 0) {
      serviceStatus = 'unknown'
      serviceLabel = 'Bez zadání'
    ...
    const summary = {
      ...
      painter_statuses_by_id: Object.fromEntries(painters.map((item) => [item.id, item.visual_status || UNKNOWN_AVAILABILITY_STATUS])),
    }
...
class JsonStateStore {
  ...
  async createJob(payload, req) {
    ...
    runInBackground('job creation emails', async () => {
  ...
  async sendOffer(jobId, painterId, adminEmail, req, durationDays = 1) {
    ...
    if (job.status === JOB_STATUSES.PAINTER_ACCEPTED || job.status === JOB_STATUSES.CLIENT_CONFIRMED) {
      throw new Error('Zakázka už má přijatého nebo potvrzeného malíře.')
    }

    const pendingOffers = state.job_offers.filter((item) => item.job_id === jobId && item.status === OFFER_STATUSES.PENDING)
    if (pendingOffers.length > 0) {
      const existingForPainter = pendingOffers.find((item) => item.painter_id === painterId)
      if (existingForPainter) {
        throw new Error('Tomuto malíři už čeká aktivní nabídka.')
      }
      throw new Error('Zakázka už čeká na reakci jiného malíře.')
    }
    ...
    if (painter.email) {
      runInBackground(`painter offer email ${offer.id}`, async () => {
  ...
  async getOfferByToken(token) {
    ...
    const isContactUnlocked =
      CONTACT_UNLOCKED_STATUSES.has(job.status) &&
      job.selected_offer_id === offer.id

    const safeJob = {
      ...job,
      client_name: isContactUnlocked ? job.client_name : '',
      client_phone: isContactUnlocked ? job.client_phone : '',
      client_email: isContactUnlocked ? job.client_email : '',
      address: isContactUnlocked ? job.address : '',
      client_address: isContactUnlocked ? job.client_address : '',
    }
  ...
  async expirePendingOffers() {
    ...
    for (const offer of state.job_offers) {
      if (offer.status !== OFFER_STATUSES.PENDING) continue
      if (!offer.expires_at) continue
      if (new Date(offer.expires_at).getTime() > now) continue

      offer.status = OFFER_STATUSES.EXPIRED
      offer.updated_at = nowIso()

      const job = state.jobs.find((item) => item.id === offer.job_id)
      if (job && job.status === JOB_STATUSES.OFFERED_TO_PAINTER) {
        job.status = JOB_STATUSES.READY_TO_OFFER
        job.assigned_painter_id = null
        job.updated_at = nowIso()
      }

      const releasedDates = releaseJobCapacityBlocks(state, offer.job_id, CAPACITY_BLOCK_TYPES.TEMPORARY_HOLD)
      if (releasedDates.length > 0) {
        state.job_events.push(createEvent({
          ...
          actorType: 'system',
          actorLabel: 'System',
          eventType: EVENT_TYPES.CAPACITY_BLOCK_RELEASED,
          payload: { blockType: CAPACITY_BLOCK_TYPES.TEMPORARY_HOLD, dates: releasedDates },
        }))
      }
      ...
      touchedJobIds.add(offer.job_id)
      expired += 1
    }

    if (expired > 0) {
      for (const jobId of touchedJobIds) {
        if (state.job_offers.some((item) => item.job_id === jobId && item.status === OFFER_STATUSES.ACCEPTED)) continue
        if (state.job_offers.some((item) => item.job_id === jobId && item.status === OFFER_STATUSES.PENDING)) continue
        const job = state.jobs.find((item) => item.id === jobId)
        if (!job) continue
        if (job.status === JOB_STATUSES.OFFERED_TO_PAINTER || job.status === JOB_STATUSES.READY_TO_OFFER) {
          job.status = JOB_STATUSES.READY_TO_OFFER
          job.assigned_painter_id = null
          job.updated_at = nowIso()
        }
  ...
  async confirmAssignment(jobId, adminEmail) {
    ...
    runInBackground(`assignment confirmation emails ${job.id}`, async () => {
  ...
  buildPainterPortalResponse(state, painter) {
    ...
    const availability = buildPainterPortalAvailability(state, painter.id, today, 180)
    ...
    safePainter.must_change_pin = isTemporaryPainterPinHash(painter.portal_token_hash)
  ...
  async getPainterSessionState(painterId, sessionVersion) {
    const state = await this.readState()
    const painter = state.painters.find((item) => item.id === painterId && item.portal_access_token === sessionVersion)
    if (!painter) return null
    return {
      painter,
      mustChangePin: isTemporaryPainterPinHash(painter.portal_token_hash),
    }
  }
  ...
  async changePainterPinBySession(painterId, sessionVersion, newPin) {
    const state = await this.readState()
    const painter = state.painters.find((item) => item.id === painterId && item.portal_access_token === sessionVersion)
    if (!painter) throw new Error('Relace malíře vypršela. Přihlaste se znovu pomocí PINu.')
    if (!/^\d{6}$/.test(String(newPin || ''))) throw new Error('Nový PIN musí mít přesně 6 číslic.')
    if (verifyPainterPin(String(newPin), painter.portal_token_hash)) {
      throw new Error('Nový PIN se musí lišit od dočasného PINu.')
    }

    painter.portal_token_hash = hashPainterPin(String(newPin), { temporary: false })
    painter.portal_access_token = randomSessionVersion()
    painter.updated_at = nowIso()
    await this.writeState(state)
  ...
  async setPainterPin(painterId, pin, adminEmail) {
    ...
    painter.portal_token_hash = hashPainterPin(String(pin), { temporary: true })
    painter.portal_access_token = randomSessionVersion()
    painter.updated_at = nowIso()
    await this.writeState(state)
    return {
  ...
  async resetSystem(adminEmail) {
    const state = await this.readState()
    const summary = {
      jobs: state.jobs.length,
      offers: state.job_offers.length,
      events: state.job_events.length,
      notifications: state.notifications.length,
      photos: state.job_photos.length,
      availability: state.painter_availability.length,
      capacityBlocks: state.painter_capacity_blocks.length,
    }
    ...
    await this.writeState(state)
    return { ok: true, summary, resetBy: adminEmail || 'system' }
  }
}
...
class SupabaseDbStore extends JsonStateStore {
  ...
  async _deleteAll(table, idColumn = 'id') {
    const { error } = await this.db.from(table).delete().not(idColumn, 'is', null)
    if (error) throw new Error(`DB delete error on ${table}: ${error.message}`)
  }

  // ---- JsonStateStore protocol ----

  // Override readState: normalise, then attach a version snapshot for optimistic locking.
  // The snapshot travels on the state object through writeState → writeRawState so each
  // write can verify rows haven't changed since this specific read.
  async readState() {
    const raw = await this.readRawState()
    const normalized = normalizeState(raw)
    normalized._versionSnapshot = {
      jobs: new Map(raw.jobs.map((j) => [j.id, j.updated_at])),
      offers: new Map(raw.job_offers.map((o) => [o.id, o.updated_at])),
    }
    return normalized
  }

  // Override writeState: skip re-normalizing so _versionSnapshot is preserved.
  async writeState(state) {
    return this._withLock(() => this.writeRawState(state))
  }

  // Versioned update: throws if the row was modified by another request since our read.
  async _updateVersioned(table, row, originalUpdatedAt) {
    ...
    if (!data || data.length === 0) {
      throw new Error('Zakázka byla mezitím změněna jiným požadavkem. Opakujte akci.')
    }
  }
  ...
  async writeRawState(state) {
    ...
    // Optimistic locking for jobs and offers.
    // _versionSnapshot is attached by readState() and carries the updated_at values seen
    // at read time. New rows (not in snapshot) → INSERT; existing rows → versioned UPDATE.
    // If another request wrote first, the UPDATE returns 0 rows and we throw a 409-style error.
    const snap = state._versionSnapshot || {}
    const jobVersions = snap.jobs || new Map()
    const offerVersions = snap.offers || new Map()

    const newJobs = jobs.filter((j) => !jobVersions.has(j.id))
    const existingJobs = jobs.filter((j) => jobVersions.has(j.id))
    const newOffers = offers.filter((o) => !offerVersions.has(o.id))
    const existingOffers = offers.filter((o) => offerVersions.has(o.id))

    const jobOps = [
      newJobs.length ? this._insert('jobs', newJobs) : Promise.resolve(),
      ...existingJobs.map((j) => this._updateVersioned('jobs', j, jobVersions.get(j.id))),
    ]
    const offerOps = [
      newOffers.length ? this._insert('job_offers', newOffers) : Promise.resolve(),
      ...existingOffers.map((o) => this._updateVersioned('job_offers', o, offerVersions.get(o.id))),
    ]
  ...
  async resetSystem(adminEmail) {
    ...
    await Promise.all([
      this._deleteAll('job_photos'),
      this._deleteAll('job_offers'),
      this._deleteAll('job_events'),
      this._deleteAll('notifications'),
      this._deleteAll('painter_availability'),
      this._deleteAll('painter_capacity_blocks'),
      this._deleteAll('jobs'),
    ])
    await this.db.from('push_subscriptions').delete().not('endpoint', 'is', null)

    const painters = state.painters.map((painter) => ({
      ...painter,
      notes: null,
      updated_at: nowIso(),
    }))
    await this._upsert('painters', painters)
    return { ok: true, summary, resetBy: adminEmail || 'system' }
  }
}
...
class DisabledStore {
  ...
  async expirePendingOffers() { this.error() }
...
export function getStore() {
  ...
  if (hasSupabaseConfig()) {
    ...
    console.log(`[store] Using ${cachedStore.constructor.name}`)
    return cachedStore
  }
  if (isProduction()) {
    console.error('[store] KRITICKÁ CHYBA: SUPABASE_URL a SUPABASE_SERVICE_ROLE_KEY nejsou nastaveny — produkce nefunguje!')
    cachedStore = new DisabledStore()
  } else {
    console.log('[store] Using DemoStore (dev/local)')
    cachedStore = new DemoStore()
  }
  return cachedStore
}
