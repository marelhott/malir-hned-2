...
function normalizeState(state) {
  ...
  state.painters = DEFAULT_PAINTERS.map((defaultPainter) => {
    ...
    const merged = {
      ...
      email: item.email || defaultPainter.email,
...
class JsonStateStore {
  ...
  async sendOffer(jobId, painterId, adminEmail, req) {
    ...
    const offerUrl = `${getAppBaseUrl(req)}/nabidka?token=${token}`
    const portalUrl = `${getAppBaseUrl(req)}/maliri/${encodeURIComponent(painter.name.toLowerCase())}`
    if (painter.email) {
  ...
  async confirmAssignment(jobId, adminEmail) {
    ...
    if (winningPainter) {
      sendPushToAll(winningPainter.id, {
        ...
        url: `${baseUrl}/maliri/${encodeURIComponent(winningPainter.name.toLowerCase())}`,
      }).catch(() => {})
    }
  ...
  async cancelAdminJob(jobId, adminEmail) {
    ...
    for (const painterId of affectedPainterIds) {
      ...
      if (painter?.email) {
        const portalUrl = `${baseUrl}/maliri/${encodeURIComponent(painter.name.toLowerCase())}`
