    ...
    <section id="objednavka" style={{ padding: '0 0 44px' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 64px' }}>
        ...
        <Card style={{ overflow: 'hidden' }}>
          ...
          <div style={{ padding: '24px 28px' }}>
            <p style={{ fontSize: 19, fontWeight: 300, color: T.textMid, lineHeight: 1.5, margin: '0 0 22px', textAlign: 'center' }}>
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(12,10,6,0.54)', backdropFilter: 'blur(4px)', WebkitBackdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: T.surface, borderRadius: T.r, boxShadow: T.panelShadow, maxWidth: 940, width: '100%', maxHeight: '88vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {/* Body */}
        <div style={{ padding: '24px 32px', display: 'flex', flexDirection: 'column', gap: 20, overflow: 'auto' }}>
          <div style={{ display: 'flex', gap: 18, alignItems: 'center' }}>
            <div style={{ width: 72, height: 72, borderRadius: 16, overflow: 'hidden', flexShrink: 0, border: `1px solid ${T.border}` }}>
            ...
            <div>
              <div style={{ fontSize: 15, fontWeight: 500, color: T.text, letterSpacing: '-0.02em', marginBottom: 4 }}>{painter.name}</div>
              <div style={{ fontSize: 13, fontWeight: 300, color: T.textMid, lineHeight: 1.6 }}>{painter.summary}</div>
            </div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <Card style={{ padding: '12px 16px' }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: T.textLight, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>Vybraný termín</div>
              <div style={{ fontSize: 14, fontWeight: 500, color: T.text }}>{dayLbl(monthIdx, day)}</div>
              <div style={{ fontSize: 12, fontWeight: 300, color: T.textMid, marginTop: 2 }}>{time}</div>
            </Card>
            <Card style={{ padding: '12px 16px' }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: T.textLight, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>Orientační cena</div>
              <div style={{ fontSize: 14, fontWeight: 500, color: T.text, letterSpacing: '-0.02em' }}>{fmtRange(priceRange)}</div>
              <div style={{ fontSize: 12, fontWeight: 300, color: T.textMid, marginTop: 2 }}>Může se upravit po zpracování.</div>
            </Card>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <button type="button" onClick={onConfirm} style={{ padding: '13px', borderRadius: T.br, background: T.accent, color: '#fff', fontSize: 14, fontWeight: 400, cursor: 'pointer', fontFamily: "'Outfit', sans-serif", border: 'none', boxShadow: `0 10px 24px ${T.accentShadow}` }}>
              Potvrdit a pokračovat
            </button>
            <button type="button" onClick={onClose} style={{ padding: '11px', borderRadius: T.br, background: 'transparent', color: T.textMid, fontSize: 14, fontWeight: 300, cursor: 'pointer', fontFamily: "'Outfit', sans-serif", border: `1px solid ${T.border}` }}>
              Zpět
            </button>
          </div>
        </div>
